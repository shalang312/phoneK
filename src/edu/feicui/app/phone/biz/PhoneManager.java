...
// 判断当前手机是否有ROOT权限
	...
	/*private static PhoneManager systemManager;
	private Context context;
	private TelephonyManager telManager;//管理电话
	private ConnectivityManager connManager;//管理网络连接
	...
	public int getPhoneCPUNumber(){
		...
		try {
			File dir=new File("/sys/devices/system/cpu");
			//此files数组里有[cpu0,cpu1,cpu2,cpu3]
//			File[] files=dir.listFiles(new CpuFilter());
	...
	public String getResolution(){
		String resolution="";
		DisplayMetrics metrics=new DisplayMetrics();
		//先通过getWindowManager()方法获取窗口，再通过getMetrics方法获取屏幕参数
		((Activity)context).getWindowManager().getDefaultDisplay().getMetrics(metrics);
		//获取屏幕宽高的像素点拼接成字符串
		resolution=metrics.widthPixels+"*"+metrics.heightPixels;
		return resolution;		
