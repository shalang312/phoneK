...
public class FileTypeUtil {
	...
	public static String[] getFileIconAndTypeName(File file){
		//iconAndTypes[0] 表示文件图标    iconAndTypes[1] 文件类型
		...
		if(file.isFile()){
			...
			int dotIndex=name.lastIndexOf(".");//获取.出现的下标位置   android.apk
			if(dotIndex<0){//例如android  dotIndex为-1
				return iconAndTypes;//只是返回一个默认值
			}
			//获取文件后缀名称
			String end=name.substring(dotIndex+1,name.length()).toLowerCase();//截取文件后缀名  android.apk  截取apk
			if(end.equals("")){
				return iconAndTypes;//只是返回一个默认值
			}
			
			//对ICON_TYPE_Table进行便利   找到对应图标以及文件类型
			for(int num=0;num<ICON_TYPE_Table.length;num++){
				//与二维数组里每一个后缀名进行比对
				if(end.equals(ICON_TYPE_Table[num][0])){
					iconAndTypes[0]=ICON_TYPE_Table[num][1];//图标
					iconAndTypes[1]=ICON_TYPE_Table[num][2];//文件类型
				}				
			}
			return iconAndTypes;//这些是返回带有数据的东西，和上面返回的默认值不一样
		}
		return iconAndTypes;		
	}
	...
	public static String getMIMEType(File file){
		String type="*/*";//找不到就直接给个默认字符串
		String fName=file.getName();//获取文件名
		//获取后缀名前的分隔符“.”在fName中的位置
		int dotIndex=fName.lastIndexOf(".");
		if(dotIndex<0){//例如：android
			return type;//随意返回一个默认字符串
		}
		//获取文件后缀名
		...
		//在MIME和文件类型的匹配表中找不到对应的MIME类型
		for(int num=0;num<MIME_Table.length;num++){
			if(end.equals(MIME_Table[num][0])){
				type=MIME_Table[num][1];
			}
		}
		return type;		
	}
	
	//MIME 底层根据后缀调用程序打开相应的文件
